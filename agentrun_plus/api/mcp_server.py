...
Quick Start Example for LLM Agents:
    ...
    # NOTE: src/ and artifacts/ are pre-created - do NOT mkdir them in code.
...
# Save output to artifacts/ for downloading (directory already exists)
...
Directory Structure:
    ...
    ├── src/                    # Uploaded files go here (pre-created by create_session)
    │   ├── data.csv           # Access as 'src/data.csv' from code
    │   └── script.py          # Access as 'src/script.py' from code
    └── artifacts/             # Save output files here (pre-created by create_session)
...
Safety Rules (enforced server-side):
    ...
    Directories:       src/ and artifacts/ are pre-created. Do NOT mkdir them.
"""
...
def create_mcp_app(backend: AgentRun, sessions: Dict[str, AgentRunSession], base_url: str = "http://localhost:8000"):
    ...
    def create_session() -> dict:
        ...
        Directory Structure:
            ...
            ├── src/                    # Upload files here (source_path) - PRE-CREATED
            │   └── your_file.py        # Accessible as 'src/your_file.py' from code
            └── artifacts/              # Download files from here (artifact_path) - PRE-CREATED
                └── output.png          # Generated by your code

        Important:
            - Both src/ and artifacts/ are created automatically by this call.
              Your code must NOT attempt to create these directories.
            - Code executes in workdir. Access uploaded files via 'src/filename.txt'.
    ...
    def execute_code(
        ...
        checks. Python errors are captured and returned in the output field.
        ...
        Safety - Blocked modules (will raise a safety error):
            ...
            Do NOT use these modules. Use only the packages listed by get_packages().

        Packages:
            Only packages already installed in the runner may be used.
            Call get_packages() to see the full list.
            Do NOT attempt to install additional packages in your code.
